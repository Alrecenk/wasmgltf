...
wasm: ${API_DIR}source/api.cpp
	emcc -std=c++17 -s EXPORT_NAME="initializeCPPAPI" -s MODULARIZE=1 -s ALLOW_MEMORY_GROWTH=1 -s MAXIMUM_MEMORY=4GB -O3 -s ASSERTIONS=1 ${API_MAIN} ${API_SRC} --post-js ${API_DIR}source/api_post.js -o ${WASM_OUT}api.js ${API_INC} -s "EXPORTED_FUNCTIONS=${EXPORTED_FUNCTIONS}" -s "EXPORTED_RUNTIME_METHODS=${EXTRA_EXPORTED_RUNTIME_METHODS}"
clean:
